<script lang="ts">
	import { enhance } from '$app/forms';
	import type { ActionData, PageData } from './$types';

	let { data, form }: { data: PageData; form: ActionData } = $props();
	let username = $state('');
	let password = $state('');
	
	// Safe access with default fallback
	const googleOAuthEnabled = data?.googleOAuthEnabled ?? false;
	const microsoftOAuthEnabled = data?.microsoftOAuthEnabled ?? false;
	const oauthEnabled = googleOAuthEnabled || microsoftOAuthEnabled;
</script>

<div class="login-container">
	<h1>Welcome to LuukAhead</h1>

	<form method="post" action="?/login" use:enhance class="auth-form">
		<label>
			Username
			<input name="username" bind:value={username} autocomplete="username" />
		</label>
		<label>
			Password
			<input type="password" name="password" bind:value={password} autocomplete="current-password" />
		</label>

	{#if form?.message}
		<p class="error">{form.message}</p>
	{/if}

	<button type="submit" class="btn-primary">Log in</button>
</form>

{#if oauthEnabled}
	<div class="oauth-buttons">
		{#if googleOAuthEnabled}
			<a href="/login/google" class="btn-oauth btn-google">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20">
					<path
						fill="#4285F4"
						d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
					/>
					<path
						fill="#34A853"
						d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
					/>
					<path
						fill="#FBBC05"
						d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
					/>
					<path
						fill="#EA4335"
						d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
					/>
				</svg>
				Log in with Google
			</a>
		{/if}
		
		{#if microsoftOAuthEnabled}
			<a href="/login/microsoft" class="btn-oauth btn-microsoft">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23 23" width="20" height="20">
					<path fill="#f3f3f3" d="M0 0h23v23H0z"/>
					<path fill="#f35325" d="M1 1h10v10H1z"/>
					<path fill="#81bc06" d="M12 1h10v10H12z"/>
					<path fill="#05a6f0" d="M1 12h10v10H1z"/>
					<path fill="#ffba08" d="M12 12h10v10H12z"/>
				</svg>
				Log in with Microsoft
			</a>
		{/if}
	</div>
{/if}

{#if oauthEnabled}
	<div class="divider">
		<span>or</span>
	</div>
{/if}

<form method="post" action="?/register" use:enhance class="auth-form">
	<label>
		Username
		<input name="username" bind:value={username} autocomplete="username" />
	</label>
	<label>
		Password
		<input type="password" name="password" bind:value={password} autocomplete="new-password" />
	</label>

	<button type="submit" class="btn-secondary">Register</button>
</form>

{#if oauthEnabled}
	<div class="oauth-buttons">
		{#if googleOAuthEnabled}
			<a href="/login/google" class="btn-oauth btn-google secondary">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20">
					<path
						fill="#4285F4"
						d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
					/>
					<path
						fill="#34A853"
						d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
					/>
					<path
						fill="#FBBC05"
						d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
					/>
					<path
						fill="#EA4335"
						d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
					/>
				</svg>
				Register with Google
			</a>
		{/if}
		
		{#if microsoftOAuthEnabled}
			<a href="/login/microsoft" class="btn-oauth btn-microsoft secondary">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23 23" width="20" height="20">
					<path fill="#f3f3f3" d="M0 0h23v23H0z"/>
					<path fill="#f35325" d="M1 1h10v10H1z"/>
					<path fill="#81bc06" d="M12 1h10v10H12z"/>
					<path fill="#05a6f0" d="M1 12h10v10H1z"/>
					<path fill="#ffba08" d="M12 12h10v10H12z"/>
				</svg>
				Register with Microsoft
			</a>
		{/if}
	</div>
{/if}
</div>

<style>
	.login-container {
		display: flex;
		flex-direction: column;
		align-items: center;
		padding: 2rem;
		max-width: 400px;
		margin: 0 auto;
	}

	h1 {
		margin-bottom: 2rem;
		text-align: center;
	}

	.auth-form {
		display: flex;
		flex-direction: column;
		gap: 1rem;
		width: 100%;
	}

	label {
		display: flex;
		flex-direction: column;
		font-weight: 600;
		gap: 0.25rem;
	}

	input {
		padding: 0.75rem;
		border: 1px solid var(--light-300);
		border-radius: var(--border-radius-small);
		font-size: 1rem;
	}

	input:focus {
		outline: none;
		border-color: var(--primary-500);
		box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1);
	}

	.error {
		color: #ef4444;
		font-size: 0.875rem;
		margin: 0;
	}

	button,
	.btn-oauth {
		padding: 0.75rem 1rem;
		border: none;
		border-radius: var(--border-radius-small);
		font-weight: 600;
		cursor: pointer;
		transition: all 0.2s ease;
		display: flex;
		align-items: center;
		justify-content: center;
		gap: 0.5rem;
		text-decoration: none;
		width: 100%;
	}

	.btn-primary {
		background: var(--primary-500);
		color: white;
	}

	.btn-primary:hover {
		background: var(--primary-600);
	}

	.btn-secondary {
		background: var(--light-200);
		color: var(--dark-700);
	}

	.btn-secondary:hover {
		background: var(--light-300);
	}

	.btn-google {
		background: white;
		color: #1f2937;
		border: 1px solid #e5e7eb;
	}

	.btn-google:hover {
		background: #f9fafb;
		border-color: #d1d5db;
	}

	.btn-microsoft {
		background: white;
		color: #1f2937;
		border: 1px solid #e5e7eb;
	}

	.btn-microsoft:hover {
		background: #f9fafb;
		border-color: #d1d5db;
	}

	.oauth-buttons {
		display: flex;
		flex-direction: column;
		gap: 0.75rem;
		width: 100%;
	}

	.btn-oauth.secondary {
		margin-top: 0;
	}

	.divider {
		display: flex;
		align-items: center;
		text-align: center;
		margin: 1.5rem 0;
		width: 100%;
	}

	.divider::before,
	.divider::after {
		content: '';
		flex: 1;
		border-bottom: 1px solid var(--light-300);
	}

	.divider span {
		padding: 0 1rem;
		color: var(--dark-400);
		font-size: 0.875rem;
	}

	.btn-oauth svg {
		flex-shrink: 0;
	}
</style>